...
SAVEPATH = "/Users/ariannaarmanetti/Desktop/CODES/inverse-spin/learning-pl/"
# **************************
...
def process_dataset(dataset, learning='lbfgs'):
    ...
    mcmc_ising_pl = mcmc_ising(JstarIsing, hstarIsing, R)
    mcmc_ising_pl.thermalize(betai=BETA_IN, betaf=BETA_F, nsweeps=NSWEEPS,
                               iicc=IICC, nb_chunks=NB_CHUNKS, verbose=True)

    log('sampling | thermalizing BC ...')
    mcmc_bc_pl = mcmc_ising(JstarBC, hstarBC, R, anisotropy=True)
    mcmc_bc_pl.thermalize(betai=BETA_IN, betaf=BETA_F, nsweeps=NSWEEPS,
                           iicc=IICC, nb_chunks=NB_CHUNKS, verbose=True)

    log('sampling | thermalizing BEG ...')
    mcmc_beg_pl = mcmc_beg(JstarBEG, hstarBEG, KstarBEG, R)
    mcmc_beg_pl.thermalize(betai=BETA_IN, betaf=BETA_F, nsweeps=NSWEEPS,
                            iicc=IICC, nb_chunks=NB_CHUNKS, verbose=True)
    ...
    sim_dict_list_ising_pl = _sample(mcmc_ising_pl, 'Ising')
    sim_dict_list_bc_pl    = _sample(mcmc_bc_pl,    'BC')
    sim_dict_list_beg_pl   = _sample(mcmc_beg_pl,   'BEG')
    ...
    for label, sim_dict_list in [('Ising', sim_dict_list_ising_pl),
                                  ('BC',    sim_dict_list_bc_pl),
                                  ('BEG',   sim_dict_list_beg_pl)]:
    ...
    for fname, obj in [('sim_dict_list_ising_pl', sim_dict_list_ising_pl),
                       ('sim_dict_list_bc_pl',    sim_dict_list_bc_pl),
                       ('sim_dict_list_beg_pl',   sim_dict_list_beg_pl)]:
        with open(dataset_folder + fname + '.pickle', 'wb') as f:
            pickle.dump(obj, f, protocol=pickle.HIGHEST_PROTOCOL)
